...
namespace Icaros.Desktop.Input {
    public class DeviceManager : MonoBehaviour {
        ...
        internal void Rescan() {
            ...
            StartCoroutine(IcarosControllerScan());
        }

        bool scanning = true;
        internal IEnumerator IcarosControllerScan()
        {
            scanning = true;
            while (scanning)
            {
                ...
                foreach (IInputDevice device in registeredInputDevices)
                {
                    if (device.GetDeviceTypeID() == ICAROS_CONTROLLER_DEVICE_ID)
                    {
                        scanning = false;
                        break;
                    }

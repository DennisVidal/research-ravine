...
public class MarchingCubes
{
    ...
    public static float surfaceLevel = 0.0f;




    static int[,] cornerIndices = new int[,]
    {
        {0, 1},
        {1, 2},
        {2, 3},
    ...
    public void MarchGrid(ProceduralTerrain terrain, Bounds bounds)
    {
        vertices.Clear();
        triangles.Clear();
        normals.Clear();
        ...
        sw.Start();
        for (int _z = 0; _z < bounds.size.z; _z++)
        {
            for (int _y = 0; _y < bounds.size.y; _y++)
            {
                for (int _x = 0; _x < bounds.size.z; _x++)
                {
                    Vector3 xyz = bounds.min + new Vector3(_x, _y, _z);
                    int x = (int)xyz.x;
                    int y = (int)xyz.y;
                    int z = (int)xyz.z;
                    Polygonise(new Vector3[]{   new Vector3(_x, _y, _z),
                                                new Vector3(_x, _y, _z+1),
                                                new Vector3(_x+1, _y, _z+1),
                                                new Vector3(_x+1, _y, _z),
                                                new Vector3(_x, _y+1, _z),
                                                new Vector3(_x, _y+1, _z+1),
                                                new Vector3(_x+1, _y+1, _z+1),
                                                new Vector3(_x+1, _y+1, _z)},
                                new Vector4[]{  new Vector4(0.0f,0.0f,0.0f,terrain.GetValue(new Vector3Int(x, y, z))),
                                                new Vector4(0.0f,0.0f,0.0f,terrain.GetValue(new Vector3Int(x, y, z+1))),
                                                new Vector4(0.0f,0.0f,0.0f,terrain.GetValue(new Vector3Int(x+1, y, z+1))),
                                                new Vector4(0.0f,0.0f,0.0f,terrain.GetValue(new Vector3Int(x+1, y, z))),
                                                new Vector4(0.0f,0.0f,0.0f,terrain.GetValue(new Vector3Int(x, y+1, z))),
                                                new Vector4(0.0f,0.0f,0.0f,terrain.GetValue(new Vector3Int(x, y+1, z+1))),
                                                new Vector4(0.0f,0.0f,0.0f,terrain.GetValue(new Vector3Int(x+1, y+1, z+1))),
                                                new Vector4(0.0f,0.0f,0.0f,terrain.GetValue(new Vector3Int(x+1, y+1, z)))});
                }
            }
        }
        sw.Stop();
        UnityEngine.Debug.Log("mc Generation took + " + sw.ElapsedMilliseconds);
    }

    public void March(List<Vector3[]> cubePositions, List<Vector4[]> cubeValues)
    {
        vertices.Clear();
        triangles.Clear();
        normals.Clear();
        ...
        {
            Polygonise(cubePositions[i], cubeValues[i]);
        }
    }


    public void Polygonise(Vector3[] cornerPositions, Vector4[] cornerValues)
    {
        int cubeIndex = 0;

        for (int i = 0; i < 8; i++)
        {
            if (cornerValues[i].w > surfaceLevel) 
            {
                cubeIndex |= 1 << i;
            }
        }
        ...
        Vector3[] lerpedVertices = new Vector3[12];

        for(int i = 0; i < 12; i++)
        {
            if((edgeTable[cubeIndex] & (1 << i)) != 0)
            {
                int v1 = cornerIndices[i, 0], v2 = cornerIndices[i, 1];
                lerpedVertices[i] = InterpolateVertices(cornerPositions[v1], cornerPositions[v2], cornerValues[v1].w, cornerValues[v2].w);
            }
        }

        for (int k = 0; k < triangleTable[cubeIndex].Length; k += 3)
        {
            int vIdx0 = AddVertex(lerpedVertices[triangleTable[cubeIndex][k]]);
            int vIdx1 = AddVertex(lerpedVertices[triangleTable[cubeIndex][k+2]]);
            int vIdx2 = AddVertex(lerpedVertices[triangleTable[cubeIndex][k+1]]);
            AddTriangle(vIdx0, vIdx1, vIdx2);
            
            Vector3 normal = Vector3.Cross(vertices[vIdx1] - vertices[vIdx0], vertices[vIdx2] - vertices[vIdx0]).normalized;
            AddNormal(vIdx0, normal);
            AddNormal(vIdx1, normal);
            AddNormal(vIdx2, normal);
        }
    }

    public Vector3 InterpolateVertices(Vector3 vertex1, Vector3 vertex2, float value1, float value2)
    {
        float lerpFactor = (surfaceLevel - value1) / (value2 - value1);
        return vertex1 + lerpFactor * (vertex2 - vertex1);
    }
    ...
    public void AddTriangle(int vertexIndex0, int vertexIndex1, int vertexIndex2)
    {
        triangles.Add(vertexIndex0);
        triangles.Add(vertexIndex1);
        triangles.Add(vertexIndex2);
    }
    public void AddNormal(int vertexIndex, Vector3 normal)
    {
        normals.Insert(vertexIndex, normal);
    }
}
*/